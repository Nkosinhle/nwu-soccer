...
export default function DashboardPage() {
  ...
  // useFetch caches the result — navigating away and back within 30s
  // returns the cached data immediately with no loading state at all.
  const { data: dashData, loading, error: fetchError } = useFetch<DashData>('/api/dashboard', { ttlMs: 30_000 })
  ...
  if (fetchError) return (
    <div className="flex items-center justify-center h-screen">
      <div className="text-center">
        <p className="text-slate-700 font-semibold mb-2">Failed to load dashboard</p>
        <p className="text-slate-400 text-sm mb-4">Check your connection and try again</p>
        <button onClick={() => window.location.reload()} className="px-4 py-2 rounded-lg text-white text-sm font-medium" style={{ background: '#4B0082' }}>Retry</button>
      </div>
    </div>
  )
    ...
    <div className="animate-fade-in">
      ...
      <div className="p-8">
        ...
        <div className="mb-6 p-5 bg-gradient-to-r from-purple-900 to-purple-700 rounded-2xl text-white">
          ...
          <h2 className="text-xl font-black">{session?.user?.name?.split(' ')[0] || 'there'}</h2>
