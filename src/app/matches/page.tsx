...
export default function MatchesPage() {
  ...
  const { data: matchesData, loading: matchesLoading, refetch: refetchMatches } =
    useFetch<any[]>('/api/matches', { ttlMs: 20000 })
  const matches = matchesData ?? []

  const { data: teamsData } = useFetch<any>('/api/teams', { ttlMs: 300000 })
  const teams = teamsData?.teams ?? (Array.isArray(teamsData) ? teamsData : [])

  // Players needed for lineup selection in Add Result
  const { data: playersData } = useFetch<any>('/api/players?limit=200', { ttlMs: 60000 })
    ...
    <div className="animate-fade-in">
      ...
      {showFixture && (
        <AddFixtureModal
          ...
          onSuccess={() => { setShowFixture(false); refetchMatches() }}
        />
      )}
      {showResult && (
        <AddResultModal
          ...
          onSuccess={() => { setShowResult(false); refetchMatches() }}
        />
      )}
    </div>
  )
